...
function loadConfig(configPath) {
  ...
  // The project the resources live in. Blank/omitted -> Global only. The picker
  // searches this project first, then falls back to Global.
  ...
  if (!Array.isArray(target.environmentFallback) || target.environmentFallback.length === 0) {
    target.environmentFallback = target.sourceEnvironment === 'Global'
      ? ['Global']
      : [target.sourceEnvironment, 'Global'];
  }
  // De-dupe (e.g. project explicitly set to Global).
  target.environmentFallback = [...new Set(target.environmentFallback)];
